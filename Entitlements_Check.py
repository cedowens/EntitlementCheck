...
s = os.listdir("/System/Applications/Utilities/")
print("=======================================/Application Utilities Directory Check============================================")
for app in s:
	if app.endswith('.app'):
		app_path = "/System/Applications/Utilities/%s/Contents/MacOS" % app
		try:
			u = os.listdir(app_path)
			for appfile in u:
				app_path2 = app_path + "/" + appfile
				xy = subprocess.getstatusoutput('codesign -d --entitlements :- "%s"' % app_path2)
				if 'com.apple.security.cs.allow-dyld-environment-variables</key><true/>' in xy[1]:
					print("\033[33m[-] POTENTIALLY INJECTABLE APP (has com.apple.security.cs-allow-dyld-environment-variables entitlement): %s\033[0m" % app_path2)
				if 'com.apple.security.cs.disable-library-validation</key><true/>' in xy[1]:
					print("\033[33m[-] Binary can load arbitrary unsigned plugins/frameworks (has com.apple.security.cs.disable-library-validation entitlement): %s\033[0m" % app_path2)
				if 'com.apple.security.get-task-allow</key><true/>' in xy[1]:
					print("\033[33m[-] Binary allows other non sandboxed processes to attach (has com.apple.security.get-task-allow entitlement): %s\033[0m" % app_path2)
				if 'com.apple.security.cs.allow-unsigned-executable-memory</key><true/>' in xy[1]:
					print("\033[33m[-] Binary allows c code patching, NSCreateObjectFileImageFromMemory, or dvdplayback framework (has com.apple.security.cs.allow-unsigned-executable-memory entitlement): %s\033[0m" % app_path2)
				if ('com.apple.security.files.downloads.read-only</key><true/>' in xy[1]) or ('com.apple.security.files.downloads.read-write</key></true>' in xy[1]):
					print("\033[33m[-] Binary may  have access to the Downloads folder (has com.apple.security.files.downloads.read-only or com.apple.security.files.downloads.read-write entitlement): %s\033[0m" % app_path2)
				if 'com.apple.security.files.all</key><true/>' in xy[1]:
					print("\033[33m[-] Binary may have access to all files (has deprecated com.apple.security.files.all entitlement): %s\033[0m" % app_path2)
				if ('com.apple.security.files.user-selected.read-only</key><true/>' in xy[1]) or ('com.apple.security.files.user-selected.read-write</key><true/>' in xy[1]):
					print("\033[33m[-] Binary may have access to files the user has selected in an open or save dialog (has com.apple.security.files.user-selected.read-only or com.apple.security.files.user-selected.read-write entitlement): %s\033[0m" % app_path2)
				if 'com.apple.private.security.clear-library-validation</key><true/>' in xy[1]:
					print("\033[33m[-] Binary can load third party plugins signed by non Apple developers (has com.apple.private.security.clear-library-validation entitlement): %s\033[0m" % app_path2)
				if 'com.apple.private.tcc.allow</key>' in xy[1]:
					print("\033[33m[-] Binary may have TCC access to some protected portions of the OS (has com.apple.private.tcc.allow entitlement): %s\033[0m" % app_path2)
				if 'kTCCServiceSystemPolicyDocumentsFolder' in xy[1]:
					print("\033[33m[-] Binary may have TCC access to ~/Documents (has kTCCServiceSystemPolicyDocumentsFolder access): %s\033[0m" % app_path2)
				if 'kTCCServiceSystemPolicyDownloadsFolder' in xy[1]:
					print("\033[33m[-] Binary may have TCC access to ~/Downloads (has kTCCServiceSystemPolicyDownloadsFolder access): %s\033[0m" % app_path2)
				if 'kTCCServiceSystemPolicyDesktopFolder' in xy[1]:
					print("\033[33m[-] Binary may have TCC access to ~/Desktop (has kTCCServiceSystemPolicyDesktopFolder access): %s\033[0m" % app_path2)
				if 'kTCCServiceSystemPolicyAllFiles' in xy[1]:
					print("\033[33m[-] Binary may have FDA access (has kTCCServiceSystemPolicyAllFiles access): %s\033[0m" % app_path2)
		except:
			pass
